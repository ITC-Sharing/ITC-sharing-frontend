...
const dateText = computed(() => formatRelativeDate(props.book.created_at))
  ...
  >
    ...
    <div class="relative h-56 bg-white p-3">
      ...
      <button
        ...
        class="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-white/80 text-gray-400 shadow hover:text-red-500 transition-colors cursor-pointer"
      >
    ...
    <div class="flex flex-1 flex-col gap-2 p-4 border-t border-[#E0E0E0]">
      <div class="flex items-start justify-between gap-2">
        <h2 class="text-base font-semibold leading-tight text-gray-900 line-clamp-2">{{ displayTitle }}</h2>
        <span v-if="book.majors" class="shrink-0 rounded-full bg-[#B8EDFF] px-2 py-0.5 text-xs font-medium text-[#0082B8]">
          {{ book.majors.acronym }}
        </span>
      </div>
