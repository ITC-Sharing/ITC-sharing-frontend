...
<template>
  ...
  <div class="w-full flex flex-col items-between justify-between gap-3">
    <div class="flex items-center justify-between gap-2">
      ...
      <IconTextButton :text="t('dashboard.books.listABook')" @click="showDonate = true">
        <template #icon>
    ...
    <template v-else>
      ...
      >
        <p class="text-gray-500 font-medium text-sm">
          {{
            bookFilter === 'request'
              ? t('dashboard.books.noPendingRequests')
              : t('dashboard.books.noDonatedYet')
          }}
        </p>
        <p class="text-gray-400 text-xs">
          {{
            bookFilter === 'request'
              ? t('dashboard.books.requestsHint')
              : t('dashboard.books.donatedHint')
          }}
        </p>
      </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:mt-0 mt-3">
          ...
          >
            <div class="flex items-center gap-2 flex-wrap">
              <p class="text-sm font-semibold text-gray-900 truncate flex-1 min-w-0">
                {{ req.book.title }}
              </p>
              <span
                :class="`shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full ${reqStatusBadge(req.status)}`"
                >{{ req.status }}</span
              >
            </div>
            <div class="rounded-xl py-3 flex flex-col gap-2">
              <BookInfoRow icon="owner" :label="t('dashboard.books.bookOwner')">
                <p class="text-sm font-semibold text-gray-900 truncate">
                  {{ req.donor.first_name }} {{ req.donor.last_name }}
                </p>
              </BookInfoRow>
            ...
            <p class="mt-auto text-xs text-right text-gray-400">
              {{ t('dashboard.books.receivedOn', { date: formatRelativeDate(req.requested_at) }) }}
            </p>
          </BookGridCard>
        </div>
      </template>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:mt-0 mt-3">
          ...
          >
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <span
                class="shrink-0 inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700"
              >
                <svg
                  class="h-3 w-3"
                  fill="none"
            ...
            <p class="mt-auto text-xs text-right text-gray-400">
              {{ t('dashboard.books.requestedOn', { date: formatRelativeDate(req.requested_at) }) }}
            </p>
          </BookGridCard>
        </div>
      </template>
      ...
      <div v-else-if="bookFilter !== 'yourBook'" class="flex flex-col gap-4 md:mt-0 mt-3">
        ...
        >
          <div class="flex items-center gap-2 flex-wrap">
            <p class="text-md font-semibold text-gray-900 truncate flex-1 min-w-0">
              {{ book.title }}
            </p>
            <span
              :class="`shrink-0 inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full ${reqStatusBadge(book.status === 'available' ? 'available' : book.status === 'donated' ? 'accepted' : 'pending')}`"
            >
              <svg
                v-if="book.status === 'donated'"
                class="h-3 w-3"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
              ...
              {{
                book.status === 'donated'
                  ? t('dashboard.books.acceptedBadge')
            ...
            <div class="flex items-center gap-2">
              ...
              >
                <img
                  ...
                  :src="book.request.requester.avatar_url"
                  class="h-full w-full object-cover"
                />
                <span v-else class="text-[9px] font-bold text-white">{{
                  reqInitials(book.request.requester.first_name, book.request.requester.last_name)
                }}</span>
              </div>
              <p class="text-xs text-gray-500">
                {{
                  t('dashboard.books.requestedBy', {
                    name: `${book.request.requester.first_name} ${book.request.requester.last_name}`,
                  })
                }}
              </p>
            </div>
            <p
              v-if="book.request.message"
              class="text-xs text-gray-600 bg-gray-50 rounded-lg px-3 py-2"
            >
              {{
                isExpanded(book.request.id)
                  ? book.request.message
                  : truncateText(book.request.message)
              }}
              <button
              ...
              >
                {{
                  isExpanded(book.request.id)
                    ? ` ${t('dashboard.books.seeLess')}`
                    : ` ${t('dashboard.books.seeMore')}`
                }}
              </button>
            </p>
          </template>
          ...
          <div
            v-else-if="book.request && book.request.status === 'accepted'"
            class="rounded-xl py-3 flex flex-col gap-2"
          >
            <BookInfoRow icon="owner" :label="t('dashboard.books.givenTo')">
              <p class="text-sm font-semibold text-gray-900 truncate">
                {{ book.request.requester.first_name }} {{ book.request.requester.last_name }}
              </p>
            </BookInfoRow>
            <BookInfoRow
              v-if="book.request.contact"
              icon="contact"
              :label="t('dashboard.books.contact')"
            >
              <a
            ...
            <p class="text-xs text-right text-gray-400">
              {{
                t('dashboard.books.donatedOn', {
                  date: formatRelativeDate(book.request.requested_at),
                })
              }}
            </p>
          </div>
          ...
          <!-- Accept / Reject -->
          <div
            v-if="book.request && book.request.status === 'pending'"
            class="mt-auto pt-2 flex gap-2"
          >
            <button
            ...
            <button
              ...
              class="flex-1 px-4 py-1.5 rounded-lg border border-gray-300 text-xs font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-60 transition-colors cursor-pointer"
            >
              {{ requestAction === book.request.id + '_d' ? '…' : t('dashboard.books.reject') }}
            </button>
          </div>
        </BookGridCard>
      </div>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:mt-0 mt-3">
        <BookGridCard v-for="book in myBooks" :key="book.id" :cover-url="book.cover_image_url">
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <span
              :class="`shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full ${book.status === 'available' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`"
            >
              {{
                book.status === 'donated'
          ...
          <div v-if="book.status === 'available'" class="mt-auto pt-2 flex gap-2">
            ...
            <button
              ...
              class="flex-1 px-4 py-1.5 rounded-lg border border-red-200 text-xs font-semibold text-red-600 hover:bg-red-50 transition-colors cursor-pointer"
            >
              {{ t('dashboard.books.delete') }}
            </button>
          </div>
          <p v-else class="mt-auto text-xs text-gray-400 italic">
    ...
    >
      <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <div class="mt-4 grid grid-cols-2 gap-3">
          ...
          >
            {{ declining ? t('dashboard.books.declining') : t('dashboard.books.decline') }}
          </button>
        </div>
      </div>
    </div>
